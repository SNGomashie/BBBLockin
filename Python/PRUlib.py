...
def init(PRU):
    if PRU == 0:
        # Open /sys/class/remoteproc/remoteproc0/state, in read/write mode
    ...
    if PRU == 1:
        # Open /sys/class/remoteproc/remoteproc1/state, in read/write mode
...
def read(PRU):
    ...
    if PRU == 0:
        ...
        if b'running' in cur_state:
            ...
            for x in range(100):
                try:
                    # Read the characer device and save the value in char_dev
                    readBuf = os.read(dev, RPMSG_BUF_SIZE)
