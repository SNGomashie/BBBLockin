...
void NCOsetfreq(struct NCO *n){
  ...
  temp_period = *(n->ptrPeriod) / 100;
  norm_period = POW / temp_period;
  n->incrementor = (uint64_t)norm_period * (uint64_t)*(n->ptrSperiod);
}
...
void NCOstep(struct NCO *n){
  NCOinterpolate(n);
  // n->sin_output = sinLUT[n->sin_accumulator >> 16];
  // n->cos_output = sinLUT[n->cos_accumulator >> 16];
