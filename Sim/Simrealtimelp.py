...
# Noise generation
np.random.seed(45)
Noise = ni * np.random.randn(len(t))
...
Fcl = 0.1 / nyq
Fch = 1 / nyq
N, Wn = signal.buttord(Fcl, Fch, 0.5, 20)
print(N, Wn)
# b, a = signal.butter(N, Wn, btype='low', analog=False)
b, a = signal.iirfilter(2, Fcl, btype='lowpass')
b, a = signal.cheby2(8, 70, Fcl, btype=)
print(b, a)
...
for s in VsigCosandNoise:
    ...
    Qvc[i] = (a[0] * Vc[i]) + (a[1] * Vc[i - 1]) + (a[2] * Vc[i - 2]) + (a[3] * Vc[i - 3]) + \
        (b[0] * Qvc[i - 1]) + (b[1] * Qvc[i - 2]) + \
        (b[2] * Qvc[i - 3]) + (b[3] * Qvc[i - 4])
    Ivs[i] = (a[0] * Vs[i]) + (a[1] * Vs[i - 1]) + (a[2] * Vs[i - 2]) + (a[3] * Vs[i - 3]) +  \
        (b[0] * Ivs[i - 1]) + (b[1] * Ivs[i - 2]) + \
        (b[2] * Ivs[i - 3]) + (b[3] * Ivs[i - 4])
...
plt.magnitude_spectrum(R, Fs=Fs, color='C1', label='filtered')
...
plt.plot(t, R, 'g-', linewidth=2, label='filtered signal')
plt.xlabel('Time [sec]')
