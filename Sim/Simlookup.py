...
i = 0
out = np.zeros(4096)
Pi = np.uint32(4096 * 65336 * Fd / Fs)
for i in range(len(out)):
    #
    Pa += np.uint32(Pi)

    Pa &= np.uint32((4096 * 65336) - 1)

    Idx = np.uint32(Pa >> 16)

    v1 = np.single(table[Idx])
    v2 = np.single(table[Idx + 1])

    fmul = np.single((Pa & 65336)/65336)

    out[i] = v1 + (v2-v1)*fmul
# precalc = np.delete(precalc, np.s_[1024:4096])
# np.savetxt('LUT.out', precalc, newline=', 0x', fmt='%4X')

J = len(out)
T = 1 / Fs
x = np.linspace(0.0, J * T, J)
Aofft = fftpack.fft(out)
...
plt.show()
plt.plot(range(len(precalc)), precalc, 'r+')
